...
4. **Flexible Additional Metadata**: Inspect 100% of provider-specific and raw survey metadata preserved automatically in `manifest.json` and `catalog.gpkg`.
5. **Atomic Updates**: Sync catalogs with upstream federal registries while maintaining timestamped rollback backups.
6. **Dry-Run & Safe Subsetting**: Preview fetch matrices (`fetch_array.csv`) before downloading or streaming tightly cropped cloud subsets directly from remote servers.
...
| :--- | :--- | :--- | :--- |
| `catalog/manifest.json` | JSON | Master registry tracking federated endpoints, bounding boxes, and project metadata | `als_finder search` |
| `catalog/catalog.gpkg` | GeoPackage | Vector polygon boundaries representing discovered LiDAR project footprints | `als_finder search` |
| `catalog/catalog.csv` | CSV | Human-readable tabular summary of discovered projects, dates, and point densities | `als_finder search` |
| `catalog/fetch_array.csv`| CSV | Dry-run matrix planning physical tile URLs, download targets, and estimated sizes | `als_finder download` |
| `data/raw/` | LAS / LAZ | Raw point cloud subsets organized in Hive hierarchy (`provider=*/dataset=*/`) | `als_finder download --execute` |
...
# Geospatial analysis & visualization libraries (bundled and configured by als-finder)
...
print("=" * 70)
print(" ALS-Finder Tutorial 01: Discovery & Retrieval Pipeline")
print("=" * 70)
...
```python
print("\n> Extracting sample Lake Tahoe ROI boundary...")
cmd_roi = [sys.executable, "-m", "als_finder.cli", "get-example-roi"]
roi_res = subprocess.run(cmd_roi, capture_output=True, text=True)
print(roi_res.stdout.strip())

roi_path = Path("ltbmu_boundary.gpkg")
roi_gdf = gpd.read_file(roi_path)

print(f"\n✓ ROI File:           {roi_path.resolve()}")
print(f"✓ Projected CRS:      {roi_gdf.crs}")
print(f"✓ Boundary Envelope:  minx={roi_gdf.total_bounds[0]:.4f}, miny={roi_gdf.total_bounds[1]:.4f}, maxx={roi_gdf.total_bounds[2]:.4f}, maxy={roi_gdf.total_bounds[3]:.4f}")
...
```python
print(f"\n> Running Federated Search across all providers...")
print(f"  Input ROI:       {roi_path}")
print(f"  Input Workspace: {workspace_dir}")
...
assert catalog_gpkg.exists(), "Search failed to produce catalog.gpkg!"
assert catalog_csv.exists(), "Search failed to produce catalog.csv!"
...
print(f"✓ Catalog GeoPackage: {catalog_gpkg}")
print(f"✓ Catalog CSV Table:  {catalog_csv}")
...
```python
# 4.1 Tabular Summary Table
catalog_df = pd.read_csv(catalog_csv)
print("\n--- Discovered LiDAR Acquisitions (Summary Table) ---")
print(catalog_df[["Provider", "Name", "Date", "PointDensity", "AreaSqKm", "SizeMB"]].head().to_string(index=False))

# 4.2 Interactive Leaflet Multi-Layer Map
...
```python
print("\n--- Inspecting Full 'Additional Metadata' Across Providers ---")
for ds in datasets:
    name = ds.get("name")
    prov = ds.get("provider")
    extra = ds.get("additional_metadata") or ds.get("raw_metadata") or {}
    print(f"\n[Dataset] {name} ({prov})")
    print(f"  Standard Fields: Date={ds.get('date')}, Density={ds.get('point_density')} pts/m², Points={ds.get('point_count')}")
    print(f"  Extra Metadata Keys ({len(extra)}): {list(extra.keys())}")
...
print('\n> Filter 5.1A: Exact Name Search (--name "CA_SierraNevada_5_2022")')
cmd_name_exact = [
    sys.executable, "-m", "als_finder.cli", "search",
    "--roi", str(roi_path),
...
# Example B: Wildcard
print('\n> Filter 5.1B: Wildcard Name Search (--name "*Tahoe*")')
cmd_name_wildcard = [
    sys.executable, "-m", "als_finder.cli", "search",
    "--roi", str(roi_path),
    "--name", "*Tahoe*",
...
print('\n> Filter 5.1C: Regex Name Search (--name "~^CA_Sierra.*")')
cmd_name_regex = [
    sys.executable, "-m", "als_finder.cli", "search",
    "--roi", str(roi_path),
    "--name", "~^CA_Sierra.*",
...
print('\n> Filter 5.2A: Open-Ended Chronological Filter (--date 2020-01-01/)')
cmd_date_open = [
    sys.executable, "-m", "als_finder.cli", "search",
    "--roi", str(roi_path),
    "--date", "2020-01-01/",
...
print('\n> Filter 5.2B: Bounded Historical Window (--date 2015-01-01/2019-12-31)')
cmd_date_range = [
    sys.executable, "-m", "als_finder.cli", "search",
    "--roi", str(roi_path),
...
print('\n> Filter 5.3A: USGS QL1 Quality Level Filter (--density QL1)')
cmd_density_ql1 = [
    sys.executable, "-m", "als_finder.cli", "search",
    "--roi", str(roi_path),
    "--density", "QL1",
...
print('\n> Filter 5.3B: Numeric Density Range Filter (--density 2/10)')
cmd_density_range = [
    sys.executable, "-m", "als_finder.cli", "search",
    "--roi", str(roi_path),
    "--density", "2/10",
...
```python
print('\n> Filter 5.4: Specific Provider Isolation (--provider USGS_EPT)')
cmd_provider = [
    sys.executable, "-m", "als_finder.cli", "search",
    "--roi", str(roi_path),
    "--provider", "USGS_EPT",
...
```python
print(f"\n> Running Atomic Catalog Update on Workspace: {workspace_dir}...")
cmd_update = [
...
print(f'\n> Step 7.1: Dry-Run Fetch Matrix Planning...')
print(f'  Target BBox:      {micro_roi}')
print(f'  Target Dataset:   CA_SierraNevada_5_2022')
print(f'  Target Workspace: {micro_workspace}')
...
subprocess.run(cmd_search_micro, capture_output=True)
...
print(f'> Step 7.2: Physical Download Execution (--execute)...')
cmd_exec_download = [
    sys.executable, "-m", "als_finder.cli", "download",
    "--roi", micro_roi,
    "--name", "CA_SierraNevada_5_2022",
...
for p in raw_laz_files:
    ...
    print(f"  Path: {p.relative_to(micro_workspace)}")
...
4. **Flexible Additional Metadata**: Inspected raw provider keys and survey properties.
5. **Controlled Subsetting**: Previewed download arrays and fetched raw spatial subsets.

👉 **Next Step**: Proceed to [Tutorial 02: Spatial Tiling & Lazy Point Cloud Streaming](./02_spatial_tiling_and_streaming.md) to explore metric spatial grids, zero-copy tile indexing, and parallel streaming into R `lidR` and PDAL pipelines!
