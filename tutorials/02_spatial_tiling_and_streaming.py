...
# %% [markdown]
# ---
# ## Step 2: Define a Multi-Dataset Region of Interest (ROI) & Leaflet Visualization
# 
...
# %% [markdown]
# ---
# ## Step 4: Metric Spatial Grid Generation (Core + Buffer Tiling)
# 
...
# %% [markdown]
# ---
# ## Step 6: Memory-Safe Single Tile "Hello World" Streaming (`fetch-tile`)
# 
...
# %% [markdown]
# ---
# ## Step 7: Memory-Safe Sequential Tile Loop (PDAL Python Workflow)
# 
...
print(f"✓ Generated self-contained R workflow script: {r_script_path}")

# If Rscript is available on this system, execute it to verify parity
if shutil.which("Rscript"):
    print("\nExecuting R script with Rscript...")
    r_res = subprocess.run(["Rscript", str(r_script_path)], capture_output=True, text=True)
    print(r_res.stdout)
    if r_res.stderr:
        print("R Logs:", r_res.stderr)
else:
    print("\n[NOTE] 'Rscript' executable not detected in current environment. R script is saved and ready for Codespaces / RStudio execution.")

# %% [markdown]
# ---
# ## Step 9: Containerized Workflows & Enforcing Strict Memory Limits (Docker & Apptainer)
# 
...
# %%
# Inspect environment for Docker runtime
docker_bin = shutil.which("docker")
print(f"✓ Docker Runtime Status: {'Available (' + docker_bin + ')' if docker_bin else 'Not detected (Container commands ready for CI / Docker hosts)'}")

# %% [markdown]
# ---
# ## Step 10: HPC Slurm Job Array Scaling (`als-finder fetch-tile`)
# 
...
# ```
# 
# Let's test the CLI `fetch-tile` command locally to verify command-line parity:
